...
### What the playthrough could not cover — since closed

At the time of writing, two of the five tours had been driven in a browser and
three had not. That gap is now closed; see §60.

## 60. The tour that pointed at the wrong button

§59 closed with an admission: three of the five first-run tours had never been
looked at on a phone, only checked by the static test that says each step
points at an element which exists. Closing that gap found a defect the static
test could not have.

### Building the fixtures properly

Reaching Acts 3, 4 and 5 honestly takes twenty-odd days of play. So the saves
were **generated by the real constructors** — `beginAct3`, `beginAct4`,
`beginAct5`, over a history produced by sixteen real days of `runDay` — written
out as JSON and injected into `localStorage`.

That detour was not fussiness. An earlier attempt at this hand-wrote a save,
and the loader repaired it straight back to a fresh game: §49's rule, sixth
instance. A save the real constructors produced cannot be wrong about its own
shape.

### The bug

`Spotlight` measures its target with `getBoundingClientRect`, which is
**viewport-relative**, and draws its panels `fixed`. Nothing scrolled the
target into view first.

On the market at 375px the first company card sits below the fold, at y=805 in
an 812-tall viewport — and a pinned "NEXT WEEK →" bar occupies that band. So
the ring was drawn around **the next-week button** while Pip said *"Every card
is a real company. Tap one to read its real numbers."*

A child following that instruction would have advanced a week.

The fix is one effect: scroll the target into view when the step changes, then
measure after paint. Once per step rather than on every measure, because
re-centring the page on a resize would yank it under a thumb. Verified: the
card moved from y=805 to y=374 and the lit band starts at 366, which is
374 − 8, the padding.

`scrollIntoView` is called optionally because **jsdom does not implement it**,
and the test has to install one — `vi.spyOn` cannot spy on a method that does
not exist. The guard was checked by deleting it and watching the test fail with
*"the tour never scrolled its first target into view"*.

### Why the static test could not have caught it

It asserts that a step's target **exists**. This target existed, was the right
element, and had a real box. The defect was entirely in *where the ring landed
relative to the viewport* — a fact about scroll position and a pinned bar, and
therefore a fact only a real layout has.

That is the honest division of labour: the static test stops a step pointing at
nothing, and nothing but a browser at a real size stops it pointing at the
wrong place.

### All five, now actually seen

| Tour | Verified |
|---|---|
| `the-stand` | three steps, ring moving 291 → 423 → 600px, bubble flipping to point down when there is no room below |
| `the-yard` | ringing the cooler among ten plots in four groups |
| `the-funding` | all three steps, each lighting the correct card — cash, then borrow, then the slice |
| `the-listing` | both steps, the float dial then the single-buyer alternative |
| `the-market` | both steps, on the Apple card and then the readiness gate — after the fix |

The listing screen also reconciled on the way past, which is §4's whole
demand: $232.54 a week × 14 weeks = $3,255.56, ÷ 1,000 pieces = **$3.26 each**,
a 30% cut is $69.76 a week, and the founder's own 70% is worth $2,278.89.

### One thing still not verified

`0 of 12 weeks done` was the market's own header while a tour ran over it, and
Pip's own guide bubble sits on that screen too — two bubbles with the same
accessible name, `"Got it"`, which cost this session two wrong readings before
